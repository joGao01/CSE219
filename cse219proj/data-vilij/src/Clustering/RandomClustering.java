...
public class RandomClustering extends Clusterer {
    ...
    public RandomClustering(DataSet dataset, Configuration config, ApplicationTemplate applicationTemplate, Drop drop) {
    ...
    public synchronized void run() {
        ...
        tocontinue.set(actualToContinue);
        int iteration = 0;
        System.out.println(actualToContinue);
        if(tocontinue()) {
            lock.lock();
            while ((iteration++ < maxIterations) & tocontinue.get()) {
                ...
                if (iteration % updateInterval == 0 || iteration == maxIterations) {
                    Platform.runLater(() -> {
                        appData.displayClusterData(dataset);
    ...
    private void assignLabels() {
        ...
        dataset.getLocations().forEach((instanceName, location) -> {
            int num =(int)(Math.random()*numberOfClusters+1);
           /* double minDistance      = Double.MAX_VALUE;
            ...
            dataset.getLabels().put(instanceName, Integer.toString(num));
        });
    }
